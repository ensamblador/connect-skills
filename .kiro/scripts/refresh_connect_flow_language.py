...
Run:
    uv run python .kiro/scripts/refresh_connect_flow_language.py            # write
    uv run python .kiro/scripts/refresh_connect_flow_language.py --dry-run  # preview
"""
...
# home, and the MCP folder itself later moved under ``.kiro/``. Resolve
# the same way ``_doc_fetcher`` does: prefer the ``.kiro/`` copy (the one
# the MCP server actually imports) and fall back to a repo-root checkout.
# Hardcoding the repo-root path silently wrote to a stale tree.
_ACTION_TYPES_CANDIDATES = (
    REPO_ROOT / ".kiro" / "connect_knowledge_mcp" / "connect_knowledge",
    REPO_ROOT / "connect_knowledge_mcp" / "connect_knowledge",
)
ACTION_TYPES_PATH = next(
    (c / "_action_types.py" for c in _ACTION_TYPES_CANDIDATES if c.is_dir()),
    _ACTION_TYPES_CANDIDATES[0] / "_action_types.py",
)
...
# Hand-authored operational knowledge, not derived from the upstream docs.
# These are observed ``CreateContactFlow`` / ``CreateContactFlowModule``
# 400s that ``validate_flow_json`` passes. It lives here, in the
# generator, because it previously lived in the generated steering file
# and every refresh silently deleted it. Add new findings here so they
# survive.
#
# Deliberately excluded from ``content_checksum``: the checksum tracks
# upstream-derived content only, so it keeps meaning "the AWS docs
# changed".
DEPLOY_GAPS_SECTION = """\
...
Regenerate with:
    uv run python .kiro/scripts/refresh_connect_flow_language.py
