...
# The connect_knowledge package ships with the connect-knowledge MCP
# server and exposes ``aws_search``. Depending on how the skills bundle
# was vendored, that server lives either beside this repo's root or
# under ``.kiro/`` — probe both rather than hard-coding one layout.
# When invoked via ``uv run --project <the mcp dir>`` the package is
# already importable and these inserts are inert; they are the
# belt-and-suspenders fallback for a bare interpreter. This file lives
# at ``.kiro/scripts/`` — two levels under the repo root.
_REPO_ROOT = Path(__file__).resolve().parents[2]
_MCP_CANDIDATES = (
    _REPO_ROOT / ".kiro" / "connect_knowledge_mcp",
    _REPO_ROOT / "connect_knowledge_mcp",
)
for _candidate in _MCP_CANDIDATES:
    if (_candidate / "connect_knowledge").is_dir():
